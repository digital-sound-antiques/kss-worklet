...
import { AudioRenderer, AudioRendererProgress, AudioRendererType } from './audio-renderer';
...
export class AudioPlayer {
  ...
  async play(args: any): Promise<void> {
    ...
    if (this._audioContext?.state == 'suspended') {
      throw new Error('AudioContext is suspended. Call resumeAudioContext() in advance within the call stack of a UI event handler.');
    }

    if (this._decoder == null) {
      this._decoder = new AudioDecoder(new Worker(this._decoderUrl));
      await this._decoder.init(this._audioContext!.sampleRate);
    } else {
      await this._decoder.abort();
    }
    await this._decoder.start(mch.port2, args);

    this._renderer ??= AudioRenderer.create(this._rendererType, this._audioContext!, this._rendererWorkletName);
